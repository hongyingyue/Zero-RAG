...
from utils.middleware import TimingMiddleware, SecurityMiddleware, RateLimitMiddleware
...
def create_app() -> FastAPI:
    ...
    # Setup logging
    setup_logging(level=settings.LOG_LEVEL, log_dir=settings.LOG_DIR, format_str=settings.LOG_FORMAT)
...
def add_middleware(app: FastAPI, settings):
    ...
    if not settings.DEBUG:
        app.add_middleware(TrustedHostMiddleware, allowed_hosts=["*"])  # Configure this properly in production
...
def add_routes(app: FastAPI, settings):
    ...
    async def health_check():
        return {"status": "healthy", "timestamp": time.time(), "version": settings.API_VERSION}
...
def add_exception_handlers(app: FastAPI):
    ...
    async def general_exception_handler(request: Request, exc: Exception):
        ...
        return JSONResponse(
            status_code=500, content={"error": "Internal server error", "status_code": 500, "timestamp": time.time()}
        )
