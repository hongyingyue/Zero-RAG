...
class AgentService:
    def __init__(self, rag_service: RAGService):
        self.llm = ChatOpenAI(
            model=settings.OPENAI_MODEL, temperature=settings.TEMPERATURE, openai_api_key=settings.OPENAI_API_KEY
        )
    ...
    def _create_agent(self) -> AgentExecutor:
        ...
        return AgentExecutor(agent=agent, tools=self.tools, max_iterations=settings.MAX_ITERATIONS, verbose=True)
    ...
    async def run(self, query: str, chat_history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        return await self.agent.ainvoke({"input": query, "chat_history": chat_history})
